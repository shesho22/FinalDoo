...
public final class ConsultarRutinaUseCase implements UseCase<RutinaDomain> {
	...
	private List<EjercicioEntity> obtenerListaDeEjercicios(RutinaDomain domain) {
		// Supongamos que en tu modelo de dominio, hay una lista de ejercicios asociada
		// a la rutina.
		List<EjercicioDomain> ejerciciosDomain = domain.getEjercicios();

		if (ejerciciosDomain != null) {
			// Si tienes una lista de ejercicios en el objeto RutinaDomain, convierte y
			// devuelve la lista.
			List<EjercicioEntity> ejerciciosEntity = convertirListaDeEjercicios(ejerciciosDomain);
			return ejerciciosEntity;
		} else {
			// Si no tienes una lista de ejercicios en el objeto RutinaDomain, crea y
	...
	private List<EjercicioEntity> convertirListaDeEjercicios(List<EjercicioDomain> ejerciciosDomain) {
		return new ArrayList<>(
				ejerciciosDomain.stream().map(EjercicioEntityMapper::convertToEntity).collect(Collectors.toList()));
	}

	private final void validarNoExistenciaRutinaConMismoNombreYEntrenador(final String nombre,
			final EntrenadorDomain entrenador) {
	...
	private final void setFactoria(final DAOFactory factoria) {
		if (UtilObjeto.esNulo(factoria)) {
			...
			throw ServiceGestorGimnasioException.crear(
					"Se ha presentado un problema tratando de llevar a cabo el resultado",
					"Se ha presentado un problema en setFactoria");
		}
